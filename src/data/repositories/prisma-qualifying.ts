...
import { QualifyingError, type CareerQualifyingData, type CareerQualifyingRepository } from "../../game/domain/qualifying-repository";
...
async function read(tx: Prisma.TransactionClient, careerId: string, eventId: string): Promise<CareerQualifyingData | null> {
    const progress = await loadProgress(tx, careerId);
    const event = progress?.events.find(e => e.id === eventId);
    const session = event?.weekend?.sessions.find(s => s.type === "QUALIFYING");
    ...
    return {
        progress, eventId, weekendId: event.weekend.id, sessionId: session.id, state,
...
export class PrismaQualifyingRepository implements CareerQualifyingRepository {
    ...
    async getQualifying(careerId: string, eventId: string) {
        ...
        return protect(() => this.client.$transaction(tx => read(tx, careerId, eventId), { isolationLevel: "RepeatableRead" }));
    }
        ...
        return protect(() => this.client.$transaction(async tx => {
            ...
            if (!before.state) {
                const created = await tx.careerQualifyingSimulation.create({
                    data: {
                        careerId, careerSessionId: before.sessionId, sessionType: "QUALIFYING", qualifyingVersion: 1, seed: BigInt(s.input.seed), stepMs: s.input.stepMs,
            ...
            const reread = await read(tx, careerId, eventId);
