...
import { battleContext, drsState, tyreCondition } from './race-view';
...
export function DriverPanel({ data, row, busy, send, rows }: {
    ...
    const neighbour = (side: 'ahead' | 'behind') => {
        ...
        return <div className={`neighbour ${fight ? 'fighting' : ''}`}><span>{t(side === 'ahead' ? 'viewer.ahead' : 'viewer.behind')}</span>{other ? <><strong style={{ borderColor: other.color }}>{other.abbreviation}</strong><em>{gap(ms)}</em></> : <strong>{t('race.noTime')}</strong>}</div>;
    };
  ...
  {e.pit && e.stint && <div className="strategy-focus"><div className="resource-heading"><h3>{t('viewer.strategy')}</h3><span>{t('pit.stint')} {format.number(e.stint.number)} · {t('pit.stops')} {format.number(e.pit.stops.length)}</span></div>
   ...
   <p className={e.pit.pendingCompound ? 'pit-pending' : 'ops-muted'}>{e.pit.pendingCompound ? t('pit.requested', { compound: t(`tyre.${e.pit.pendingCompound}`), lap: format.number(s.lap + 1) }) : t(row.status === 'RETIRED' ? 'incident.RETIRED' : row.status === 'FINISHED' ? 'pit.finished' : 'pit.onTrack')}</p>
