...
At export, all **293 offline tests** and **131 actual PostgreSQL integration tests** passed. Prisma format/validate/generate, TypeScript, lint and the production build passed. All seven checked-in migrations applied on a disposable local PostgreSQL database; previous migrations remain unchanged. Browser checks covered version-4 starts, pit request/change/cancel, pending refresh, tyre resets, position loss, multiple stints, AI stops, language isolation and persisted final history. See `docs/phase-8-report.md`.
