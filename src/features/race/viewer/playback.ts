...
export class PlaybackController {
    ...
    private snapshot: PlaybackSnapshot = { playing: false, busy: false, speed: 1, autoPause: true, skipping: false, reason: null, error: null };
    ...
    pause = () => { this.clear(); this.skipRemaining = 0; this.emit({ playing: false, skipping: false }); };
    play = () => { if (this.state.status === 'FINISHED')
        return; this.emit({ playing: true, reason: null, error: null }); this.schedule(); };
    ...
    skip = () => { if (this.state.status === 'FINISHED')
        return; this.skipRemaining = 20; this.emit({ playing: true, skipping: true, reason: null, error: null }); this.clear(); this.schedule(); };
    step = async () => { this.pause(); await this.tick(); };
    private schedule() { if (!this.snapshot.playing || this.snapshot.busy || this.timer !== null || this.state.status === 'FINISHED')
        return; this.timer = this.clock.set(() => { this.timer = null; void this.tick(); }, this.snapshot.skipping ? 50 : 2400 / this.snapshot.speed); }
    private async tick() {
