...
3. **Stable replacement (stability repair 2).** A genuine conflict (map edge, START / FINISH, or a higher- or equal-priority label) moves the label immediately. The new slot is not simply the first clean one. Each of the 24 candidates (8 directions at 3 radii) that is hard-valid is scored:
   ...
   **Lookahead (START / FINISH repair).** Each labelled, moving car also carries its next positions along the drawn path: 13 samples, 25 SVG units apart (325 units), keeping its lane offset. For every candidate the scoring asks how far along that lookahead the slot survives: it must stay inside the map, clear of START / FINISH and clear of higher-priority labels' own lookahead positions, checked with an 8-unit margin. A slot that survives the whole lookahead costs nothing. Otherwise, the earlier its first conflict, the larger the penalty. This outweighs current-frame clearance, so one move carries a label through a whole diagonal crossing. The lookahead only chooses among slots that are valid now; current-frame hard conflicts still move a label immediately.
