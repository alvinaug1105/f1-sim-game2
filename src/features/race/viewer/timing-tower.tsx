...
/**
 * The whole row is one selection target (click anywhere; the driver button is the single keyboard/screen-reader
...
export function TimingTower({ state: s, rows, selected, onSelect, interval, onInterval, attentionId = null }: { state: RaceSimulationState; rows: Rows; selected: string; onSelect: (id: string) => void; interval: boolean; onInterval: (interval: boolean) => void; attentionId?: string | null }) {
    ...
    return <section className="ops-panel timing-panel" aria-label={t('viewer.timing')}>
        ...
        <div className="timing-scroll"><table className="timing-tower"><caption className="sr-only">{title} · {t(interval ? 'viewer.interval' : 'viewer.gap')}</caption>
            <thead><tr><th>{t('race.position')}</th><th>{t('race.driver')}</th><th>{t(interval ? 'viewer.interval' : 'viewer.gap')}</th><th className="col-last">{t('viewer.lastLap')}</th>{s.input.tyres && <th>{t('viewer.tyre')}</th>}</tr></thead>
            <tbody>{rows.map(r => {
                ...
                return <tr key={r.id} onClick={() => onSelect(r.id)} data-entrant={r.id} className={['tower-row', chosen && 'selected-row', r.player && 'player-row', retired && 'retired-row', r.pitting && 'pit-row'].filter(Boolean).join(' ')} style={r.player ? { ['--team' as string]: r.color } : undefined}>
                    ...
                    <td className="col-last">{last === null ? t('race.noTime') : <>{formatRaceTime(last, locale)}{best !== null && last === best && <small className="pb-mark">{t('viewer.personalBest')}</small>}</>}</td>
